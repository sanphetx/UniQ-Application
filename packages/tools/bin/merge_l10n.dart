...
Future<void> main(List<String> args) async {
  final targetDir = Directory('lib/src/generated/localization');
...
Future<List<String>> _buildSearchDirs() async {
  final modulesDir = Directory('lib/src/presentation/screens');
  ...
  await for (var entity in modulesDir.list(recursive: true)) {
    if (entity is Directory && entity.path.endsWith('localization')) {
...
Future<void> _collectArbFiles(Directory dir, Map<String, List<File>> localizationFiles) async {
  await for (final entity in dir.list(recursive: true, followLinks: false)) {
    if (entity is File) {
      ...
      final match = RegExp(r'app_([a-z]{2})\.arb').firstMatch(fileName);
